package com.pharma.medicinestock.scheduler;

import com.pharma.medicinestock.repository.MedicineStockAdjustmentRepository;
import lombok.RequiredArgsConstructor;
import org.springframework.scheduling.annotation.Scheduled;
import org.springframework.stereotype.Component;
import org.springframework.transaction.annotation.Transactional;

import java.time.LocalDateTime;
import java.time.ZoneId;

@Component
@RequiredArgsConstructor
public class MedicineStockAdjustmentScheduler {

    private final MedicineStockAdjustmentRepository medicineStockAdjustmentRepository;

    /** Runs every hour; flips inTransit → false for each adjustment whose transitDays have elapsed. */
    @Scheduled(cron = "0 0 * * * *")
    @Transactional
    public void expireInTransitAdjustments() {
        LocalDateTime now = LocalDateTime.now(ZoneId.of("Asia/Kolkata"));
        var expired = medicineStockAdjustmentRepository.findAllActiveInTransit().stream()
                .filter(a -> a.getAdjustedAt().plusDays(a.getTransitDays()).isBefore(now))
                .peek(a -> a.setInTransit(false))
                .toList();
        if (!expired.isEmpty()) {
            medicineStockAdjustmentRepository.saveAll(expired);
        }
    }
}
