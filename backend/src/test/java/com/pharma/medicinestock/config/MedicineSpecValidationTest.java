...
import com.pharma.medicinestock.entity.Medicine.MedicineType;
...
import static com.pharma.medicinestock.entity.MedicineStock.MedicineStockType.*;
...
class MedicineSpecValidationTest {
    ...
    class SeedState {
        ...
        @Test @DisplayName("No lostmedicineStock system user exists after seed")
        void noSystemUserAfterSeed() {
            boolean exists = userRepository.findByUsername("lostmedicineStock").isPresent();
            assertThat(exists).as("lostmedicineStock system user should not exist").isFalse();
        }
        ...
        @Test @DisplayName("Seed creates 15 medicineStock records: 5 admin stock + 5 john.doe + 5 jane.smith")
        void medicineStockSeededCorrectly() {
            var all = medicineStockRepository.findAll();
            assertThat(all).hasSize(15);
            long adminRows = all.stream()
                .filter(i -> i.getMedicineStockType() == MedicineStock.MedicineStockType.ADMIN_MEDICINE_STOCK)
                .count();
            long userRows = all.stream()
                .filter(i -> i.getMedicineStockType() == MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK)
                .count();
    ...
    /**
     * These tests specifically guard against the production bug where medicine_stock_type values
     ...
     */
    @Nested @DisplayName("MedicineStock JPQL enum queries")
    class MedicineStockEnumQueries {
        ...
        void regularStockIsReturnedByJpqlEnumQuery() {
            List<MedicineStock> rows = medicineStockRepository.findAllNonZeroOrderByMedicineAndUser(REGULAR_MEDICINE_STOCK);
            assertThat(rows)
                .as("JPQL query for REGULAR_MEDICINE_STOCK must return rows — if empty, medicine_stock_type " +
        ...
        void adminStockIsReturnedByJpqlEnumQuery() {
            List<MedicineStock> rows = medicineStockRepository.findAllNonZeroOrderByMedicineAndUser(ADMIN_MEDICINE_STOCK);
            assertThat(rows)
                .as("JPQL query for ADMIN_MEDICINE_STOCK must return rows — if empty, medicine_stock_type " +
        ...
        @Test @DisplayName("findAllNonZeroRegularForValuation returns regular rows (used by medicine-stock-valuation report)")
        void valuationQueryWorksForRegularStock() {
            List<MedicineStock> rows = medicineStockRepository.findAllNonZeroRegularForValuation(REGULAR_MEDICINE_STOCK);
            assertThat(rows)
        ...
        void regularStockRowsHavePositiveQuantity() {
            medicineStockRepository.findAllNonZeroOrderByMedicineAndUser(REGULAR_MEDICINE_STOCK)
        ...
        void adminStockRowsHavePositiveQuantity() {
            medicineStockRepository.findAllNonZeroOrderByMedicineAndUser(ADMIN_MEDICINE_STOCK)
                .forEach(i -> assertThat(i.getQuantity()).as("quantity must be positive").isPositive());
        }
    }
