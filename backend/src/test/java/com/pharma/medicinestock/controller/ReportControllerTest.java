...
class ReportControllerTest {
    ...
    @Nested @DisplayName("GET /api/reports/medicine-stock-by-user")
    class MedicineStockByUser {

        @Test @WithMockUser(roles = "ADMIN")
        void adminCanGetMedicineStockByUser() throws Exception {
            when(reportService.medicineStockByUser()).thenReturn(sampleReport("MEDICINE_STOCK_BY_USER"));
            mockMvc.perform(get("/api/reports/medicine-stock-by-user"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.reportType").value("MEDICINE_STOCK_BY_USER"))
                    .andExpect(jsonPath("$.content").value("Sample report content for MEDICINE_STOCK_BY_USER"));
        }
    ...
    @Nested @DisplayName("GET /api/reports/medicine-stock-valuation")
    class MedicineStockValuation {

        @Test @WithMockUser(roles = "ADMIN")
        void adminCanGetValuation() throws Exception {
            when(reportService.medicineStockValuation((LocalDate) null)).thenReturn(sampleReport("MEDICINE_STOCK_VALUATION"));
            mockMvc.perform(get("/api/reports/medicine-stock-valuation"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.reportType").value("MEDICINE_STOCK_VALUATION"));
        }

        @Test @WithMockUser(roles = "ADMIN")
        void adminCanGetValuationWithDateParam() throws Exception {
            when(reportService.medicineStockValuation(ArgumentMatchers.any(LocalDate.class)))
                    .thenReturn(sampleReport("MEDICINE_STOCK_VALUATION"));
            mockMvc.perform(get("/api/reports/medicine-stock-valuation").param("date", "2026-05-01"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.reportType").value("MEDICINE_STOCK_VALUATION"));
        }
        ...
        void adminCanGetValuationWithoutDateParam() throws Exception {
            when(reportService.medicineStockValuation((LocalDate) null))
                    .thenReturn(sampleReport("MEDICINE_STOCK_VALUATION"));
            mockMvc.perform(get("/api/reports/medicine-stock-valuation"))
                    .andExpect(status().isOk());
        }
