package com.pharma.medicinestock.mapper;
...
class TransactionMapperTest {
    ...
    @Test @DisplayName("defaults medicineStockType to REGULAR_MEDICINE_STOCK when null (backward compatible with old records)")
    void toResponse_nullMedicineStockType_defaultsToRegular() {
        Transaction t = buildTx(TransactionStatus.PENDING);
        t.setMedicineStockType(null);

        TransactionResponse r = mapper.toResponse(t);
        assertThat(r.getMedicineStockType()).isEqualTo("REGULAR_MEDICINE_STOCK");
    }

    @Test @DisplayName("maps explicit REGULAR_MEDICINE_STOCK medicineStockType")
    void toResponse_regularMedicineStockType_mapsCorrectly() {
        Transaction t = buildTx(TransactionStatus.PENDING);
        t.setMedicineStockType(MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK);

        TransactionResponse r = mapper.toResponse(t);
        assertThat(r.getMedicineStockType()).isEqualTo("REGULAR_MEDICINE_STOCK");
    }

    @Test @DisplayName("maps explicit ADMIN_MEDICINE_STOCK medicineStockType")
    void toResponse_adminMedicineStockType_mapsCorrectly() {
        Transaction t = buildTx(TransactionStatus.PENDING);
        t.setMedicineStockType(MedicineStock.MedicineStockType.ADMIN_MEDICINE_STOCK);

        TransactionResponse r = mapper.toResponse(t);
        assertThat(r.getMedicineStockType()).isEqualTo("ADMIN_MEDICINE_STOCK");
    }
}
