...
class TransactionServiceTest {
    ...
    void setUp() {
        ...
        medicineStock.setId(1L); medicineStock.setUser(regularUser);
        medicineStock.setMedicine(medicine); medicineStock.setQuantity(BigDecimal.valueOf(50));

        // Default: settled quantity mirrors whatever the test set medicineStock.quantity to —
        ...
        lenient().when(currentStockCalculator.settledQuantity(any(), any(), any()))
                .thenAnswer(inv -> medicineStock.getQuantity());
    }
    ...
    class Submit {
        ...
        @Test @DisplayName("creates PENDING transaction and deducts medicineStock")
        void submit_valid_createsPendingAndDeducts() {
            ...
            when(medicineStockRepository.findByUserIdAndMedicineIdAndMedicineStockType(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
            ...
            verify(medicineStockRepository).save(argThat(i -> i.getQuantity().compareTo(BigDecimal.valueOf(40)) == 0));
        }
        ...
        @Test @DisplayName("throws ResourceNotFoundException when no medicineStock for medicine")
        void submit_medicineStockNotFound_throwsResourceNotFound() {
            when(userRepository.findByUsername("john.doe")).thenReturn(Optional.of(regularUser));
            when(medicineRepository.findById(1L)).thenReturn(Optional.of(medicine));
            when(medicineStockRepository.findByUserIdAndMedicineIdAndMedicineStockType(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
            ...
            assertThatThrownBy(() ->
                    ...
                    .isInstanceOf(ResourceNotFoundException.class)
                    .hasMessageContaining("MedicineStock");
        }

        @Test @DisplayName("throws InsufficientMedicineStockException when qty exceeds stock")
        void submit_insufficientStock_throwsInsufficientMedicineStock() {
            medicineStock.setQuantity(BigDecimal.valueOf(5));
        ...
        @Test @DisplayName("throws InsufficientMedicineStockException when requested exceeds the reconstructed settled quantity, even though raw medicineStock.quantity is enough")
        void submit_exceedsSettledStock_throwsInsufficientMedicineStockDespiteRawQuantity() {
            ...
            medicineStock.setQuantity(BigDecimal.valueOf(11));
            ...
            when(medicineStockRepository.findByUserIdAndMedicineIdAndMedicineStockType(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
                    .thenReturn(Optional.of(medicineStock));
            when(currentStockCalculator.settledQuantity(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
        ...
        void submit_exactlySettledStock_succeeds() {
            medicineStock.setQuantity(BigDecimal.valueOf(11)); // raw 11, reconstructed settled 8
            ...
            assertThatNoException().isThrownBy(() -> transactionService.submit(req, "john.doe"));
            verify(medicineStockRepository).save(argThat(i -> i.getQuantity().compareTo(BigDecimal.valueOf(3)) == 0)); // raw 11 - 8
        }
    }
    ...
    class Approve {
        ...
        @Test @DisplayName("sets status REJECTED and restores medicineStock")
        void reject_valid_setsRejectedAndRestoresMedicineStock() {
            when(transactionRepository.findById(1L)).thenReturn(Optional.of(pendingTx));
            when(userRepository.findByUsername("admin")).thenReturn(Optional.of(adminUser));
            when(medicineStockRepository.findByUserIdAndMedicineIdAndMedicineStockType(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
                    .thenReturn(Optional.of(medicineStock));
            ...
            verify(medicineStockRepository).save(argThat(i -> i.getQuantity().compareTo(BigDecimal.valueOf(60)) == 0)); // 50+10
        }
    ...
    class DeleteTransaction {
        ...
        @Test @DisplayName("deletes PENDING transaction and restores medicineStock")
        void delete_pendingTx_restoresMedicineStockAndDeletes() {
            when(transactionRepository.findById(1L)).thenReturn(Optional.of(pendingTx));
            when(medicineStockRepository.findByUserIdAndMedicineIdAndMedicineStockType(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
                    .thenReturn(Optional.of(medicineStock));
            when(medicineStockRepository.save(any())).thenReturn(medicineStock);
        ...
        @Test @DisplayName("deletes APPROVED transaction and restores medicineStock")
        void delete_approvedTx_restoresMedicineStockAndDeletes() {
            pendingTx.setStatus(TransactionStatus.APPROVED);
            when(transactionRepository.findById(1L)).thenReturn(Optional.of(pendingTx));
            when(medicineStockRepository.findByUserIdAndMedicineIdAndMedicineStockType(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
                    .thenReturn(Optional.of(medicineStock));
            ...
            verify(medicineStockRepository).save(argThat(i -> i.getQuantity().compareTo(BigDecimal.valueOf(60)) == 0));
            verify(transactionRepository).delete(pendingTx);
        }

        @Test @DisplayName("deletes REJECTED transaction without restoring medicineStock")
        void delete_rejectedTx_doesNotRestoreMedicineStock() {
            pendingTx.setStatus(TransactionStatus.REJECTED);
            when(transactionRepository.findById(1L)).thenReturn(Optional.of(pendingTx));
    ...
    class DeleteOwnPending {
        ...
        @Test @DisplayName("owner deletes their own PENDING transaction and medicineStock is restored")
        void delete_ownPendingTx_restoresMedicineStockAndDeletes() {
            when(transactionRepository.findById(1L)).thenReturn(Optional.of(pendingTx));
            when(userRepository.findByUsername("john.doe")).thenReturn(Optional.of(regularUser));
            when(medicineStockRepository.findByUserIdAndMedicineIdAndMedicineStockType(1L, 1L, MedicineStock.MedicineStockType.REGULAR_MEDICINE_STOCK))
                    .thenReturn(Optional.of(medicineStock));
