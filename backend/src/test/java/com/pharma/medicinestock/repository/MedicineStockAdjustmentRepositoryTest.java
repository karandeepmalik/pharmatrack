...
@DataJpaTest
@DisplayName("MedicineStockAdjustmentRepository JPQL integration")
class MedicineStockAdjustmentRepositoryTest {

    @Autowired TestEntityManager em;
    @Autowired MedicineStockAdjustmentRepository repo;

    private User user1;
    private User admin;
    private Medicine medicine;

    private MedicineStockAdjustment adj1; // Jan 1, inTransit=false, adjustedBy=null
    private MedicineStockAdjustment adj2; // Jan 2, inTransit=true,  adjustedBy=null
    private MedicineStockAdjustment adj3; // Jan 3, inTransit=false, adjustedBy=admin
    ...
    class FindByDateRange {
        ...
        void returnsWithinRange() {
            ...
            List<MedicineStockAdjustment> result = repo.findByDateRange(
                    LocalDateTime.of(2024, 1, 1, 0, 0),
                    LocalDateTime.of(2024, 1, 3, 0, 0));
            assertThat(result).extracting(MedicineStockAdjustment::getId)
    ...
    class FindAllActiveInTransit {
        ...
        void returnsOnlyInTransit() {
            List<MedicineStockAdjustment> result = repo.findAllActiveInTransit();
            assertThat(result).extracting(MedicineStockAdjustment::getId).containsExactly(adj2.getId());
        }
    ...
    class FindAllUpToForUser {
        ...
        void returnsBeforeEndForUser() {
            ...
            List<MedicineStockAdjustment> result = repo.findAllUpToForUser(
        ...
        void excludesAtOrAfterEnd() {
            List<MedicineStockAdjustment> result = repo.findAllUpToForUser(user1.getId(), JAN_3);
            assertThat(result).noneMatch(a -> a.getId().equals(adj3.getId()));
        }
    ...
    class FindAllUpTo {
        ...
        void returnsBeforeEnd() {
            ...
            List<MedicineStockAdjustment> result = repo.findAllUpTo(
                    LocalDateTime.of(2024, 1, 3, 0, 0));
            assertThat(result).extracting(MedicineStockAdjustment::getId)
                    .containsExactlyInAnyOrder(adj1.getId(), adj2.getId());
        }
        ...
        @Test @DisplayName("excludes record at or after endExclusive")
        void excludesAtOrAfterEnd() {
            List<MedicineStockAdjustment> result = repo.findAllUpTo(JAN_3);
            assertThat(result).noneMatch(a -> a.getId().equals(adj3.getId()));
        }
    ...
    class FindWithDetailsBetween {
        ...
        void returnsWithinRange() {
            List<MedicineStockAdjustment> result = repo.findWithDetailsBetween(
                    LocalDateTime.of(2024, 1, 1, 0, 0),
                    LocalDateTime.of(2024, 1, 4, 0, 0));
            assertThat(result).extracting(MedicineStockAdjustment::getId)
                    .containsExactlyInAnyOrder(adj1.getId(), adj2.getId(), adj3.getId());
        ...
        void eagerlyLoadsAdjustedBy() {
            List<MedicineStockAdjustment> result = repo.findWithDetailsBetween(
                    LocalDateTime.of(2024, 1, 1, 0, 0),
                    LocalDateTime.of(2024, 1, 4, 0, 0));
            MedicineStockAdjustment withAdmin = result.stream()
        ...
        void adjustedByNullForNonAdminRecords() {
            List<MedicineStockAdjustment> result = repo.findWithDetailsBetween(
                    LocalDateTime.of(2024, 1, 1, 0, 0),
                    LocalDateTime.of(2024, 1, 4, 0, 0));
            MedicineStockAdjustment withoutAdmin = result.stream()
    ...
    class NullifyAdjustedBy {
        ...
        void nullifiesAdjustedBy() {
            ...
            MedicineStockAdjustment found = em.find(MedicineStockAdjustment.class, adj3.getId());
        ...
        void doesNotAffectOtherRecords() {
            ...
            assertThat(em.find(MedicineStockAdjustment.class, adj1.getId()).getAdjustedBy())
        ...
        void noopWhenNoMatch() {
            ...
            assertThat(em.find(MedicineStockAdjustment.class, adj3.getId()).getAdjustedBy()).isNotNull();
        }
    }
    ...
    private MedicineStockAdjustment buildAdj(User user, LocalDateTime adjustedAt,
