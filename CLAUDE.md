...
### Auth
JWT (HS256, self-issued), returned in the login response body and kept by the frontend in `localStorage` for the `Authorization: Bearer` header via an Axios interceptor in `frontend/src/api/api.js` — a 401 clears it and redirects to `/login`, **except** a 401 from `/auth/login` itself (wrong credentials, not session expiry — the interceptor checks `error.config.url`, since redirecting there would hard-reload the login page and wipe out the error message before the user ever sees it; this was a real bug, found by the Playwright suite in `e2e-browser/`). There is no cookie: frontend and backend are separate origins (different Cloud Run services), and a `SameSite=Lax` cookie is never sent on a cross-origin XHR/fetch — only a top-level GET navigation — so a cookie-based approach was a genuine dead end here, not a style preference. (An earlier version of this app *did* also set an HttpOnly cookie "for defence-in-depth," but it was never actually reachable in this deployment topology; it was removed, along with the now-pointless `withCredentials`/`allowCredentials` CORS settings, once that was noticed.) `LoginRateLimiter` tracks failures on two independent dimensions (per-username and per-IP via `X-Forwarded-For`, since Cloud Run always proxies) — a successful login resets both. `DataInitializer`'s demo-account seeding is gated `@Profile("!test & !prod")` and requires `SPRING_PROFILES_ACTIVE=prod` to actually take effect in production — it will NOT skip prod without that env var being set correctly.
...
- Redis (login rate-limit counters, see Auth below) is self-hosted on this same VM — password-protected, `maxmemory 64mb` + `allkeys-lru` so it can't pressure the VM's 1 GB RAM alongside Postgres. Reached the same way as Postgres (same VPC, same firewall coverage), no dedicated networking of its own.
