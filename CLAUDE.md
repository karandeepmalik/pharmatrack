...
- Before merging: run the full suite (backend, frontend, e2e; e2e-browser too for frontend-behavior changes) locally, then push/open the PR and wait for GitHub Actions CI to go green too. If everything passes, squash-merge and delete the branch — no need to ask each time. Merging to `main` deploys to Cloud Run, so confirm the deployed service is actually healthy (`/actuator/health`) before considering the task done; a green test suite does not guarantee a working deploy.
...
- **`e2e-browser/` test data accumulates in the local dev DB across runs** (dispatches, users, medicines never get cleaned up) — `make down-v` wipes it via `DataInitializer` reseeding. Tests that look up a just-created item scroll (via the `scrollUntilVisible` helper in `tests/helpers.js`) rather than assuming it lands on page 0, since PAGE_SIZE=10/20 lists sort by `submittedAt` and many same-day items (dispatch date, not wall-clock time, drives `submittedAt`) can tie and push a specific item past the first page.
